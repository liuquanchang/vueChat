<template>
  <div class="login_box">
    <div class="login_">
      ...
      <button class="l_but" @click="loginUser">登 录</button>
      <div class="l_link">
  ...
  export default{
    ...
    methods:{
              loginUser(){
                  ...
                  this.send_interface.userCtx.login(data, (dataMsg)=> {
                    if(dataMsg.status===10000){
                      lockr.set(cacheKey.userInfo,JSON.stringify({username:data.username,userId:data.userId}));
                      //编程式跳转
